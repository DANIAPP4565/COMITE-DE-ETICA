...
## Corrección del error `No module named cei_core.audit`

La aplicación ya no depende de `cei_core.audit` para iniciar: las funciones de anclaje de auditoría también están integradas en `app.py`. No obstante, el directorio `cei_core` completo debe permanecer en el repositorio porque contiene la base de datos, MFA, almacenamiento, firma, reportes y workflow.

Antes de volver a desplegar en Streamlit Cloud:
...
```text
OK: estructura e importaciones completas.
```

En GitHub, `app.py`, `requirements.txt` y la carpeta `cei_core/` deben encontrarse en el mismo nivel. No cargue solamente `app.py`.

### Secretos de Streamlit Cloud

Copie el contenido de `.streamlit/secrets.toml.example` en **App settings → Secrets** y reemplace todos los valores de ejemplo. El código puede leer la configuración desde variables de entorno o desde `st.secrets`.
