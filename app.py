...
def create_audit_anchor(user_id: int) -> dict:
    """Seal the current audit-chain head in encrypted WORM-capable storage.

    Kept in app.py as well as cei_core.audit so a partial repository update cannot
    break application startup because of one missing optional service module.
    """
    verification = verify_audit_chain()
    if not verification.get("valid"):
        raise RuntimeError(
            "La cadena de auditoría no es válida. "
            f"Primer registro inconsistente: {verification.get('bad_id')}"
        )
    latest = one("SELECT id, entry_hash, created_at FROM audit_log ORDER BY id DESC LIMIT 1")
    if not latest:
        raise RuntimeError("No hay eventos de auditoría para sellar.")
    existing = one("SELECT id FROM audit_anchors WHERE audit_log_id=?", (latest["id"],))
    if existing:
        return {"already_exists": True, "anchor_id": existing["id"], **latest}

    payload = {
        "format": "CEI-AUDIT-ANCHOR-1",
        "audit_log_id": latest["id"],
        "event_count": verification["checked_entries"],
        "chain_hash": latest["entry_hash"],
        "audit_created_at": latest["created_at"],
    ...
    stored = get_storage().put_encrypted(
        data=data,
        namespace="audit-anchors",
        filename=f"audit-anchor-{latest['id']}.json",
        content_type="application/json",
        aad={"record_type": "audit_anchor", "audit_log_id": latest["id"]},
        retention_days=3650,
        legal_hold=True,
    )
    anchor_id = execute(
        """INSERT INTO audit_anchors
        (audit_log_id, event_count, chain_hash, object_key, object_version, created_by, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (
            latest["id"], verification["checked_entries"], latest["entry_hash"],
            stored.object_key, stored.version_id, user_id, payload["anchored_at"],
        ),
    )
    log_action(user_id, "Selló cadena de auditoría", "audit_anchor", anchor_id, payload)
    return {"already_exists": False, "anchor_id": anchor_id, **payload}


def list_audit_anchors() -> list[dict]:
    return query(
        """SELECT a.*, u.full_name AS created_by_name
           FROM audit_anchors a LEFT JOIN users u ON u.id=a.created_by
